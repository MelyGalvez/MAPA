  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  HAL_ADC_PollForConversion(&hadc1, 20);
	   if (i < AUDIO_BUFFER_SIZE && dataSent == 0) {
	 	  audioSource[i] = HAL_ADC_GetValue(&hadc1);
	   	  i++;
	   	  dataSent = 0;
	   }
	   if (i >= AUDIO_BUFFER_SIZE) {
	       Process_Audio();      // Traiter les données audio
	       Send_Frequency(freqMax); // Envoyer la fréquence maximale détectée
	       dataSent = 1;         // Indiquer que les données ont été transmises
	       i = 0;                // Réinitialiser l'indice
	     }
	     for (int j = 0; j < AUDIO_BUFFER_SIZE; j++) {
	     	sprintf(msg, "audioSource[%d] = %d\r\n", j, audioSource[j]);
